...
class ParticleSwarmOptimization:
    ...
    def __call__(self, func):
        ...
        w_initial = 0.9
        w_final = 0.4
        c1_initial, c2_initial = 2.5, 0.5  # Adjusted initial values
        c1_final, c2_final = 0.5, 2.5  # Adjusted final values
        
        evals = 0
        
        while evals < self.budget:
            inertia_weight = w_initial - (w_initial - w_final) * (evals / self.budget)
            c1 = c1_initial - (c1_initial - c1_final) * (evals / self.budget)  # Dynamic c1
            c2 = c2_initial + (c2_final - c2_initial) * (evals / self.budget)  # Dynamic c2
