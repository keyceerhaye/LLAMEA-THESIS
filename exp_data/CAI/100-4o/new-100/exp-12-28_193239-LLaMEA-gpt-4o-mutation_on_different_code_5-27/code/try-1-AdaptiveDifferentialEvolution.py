    ...
    def __call__(self, func):
        ...
        for _ in range(self.budget - self.pop_size):
            for j in range(self.pop_size):
                ...
                # Change 1: Added scaling factor based on fitness
                fitness_diff = (fitness[j] - self.f_opt) / max(abs(fitness[j]), 1e-8)
                mutant = np.clip(a + self.f * (b - c) * fitness_diff, func.bounds.lb, func.bounds.ub)
