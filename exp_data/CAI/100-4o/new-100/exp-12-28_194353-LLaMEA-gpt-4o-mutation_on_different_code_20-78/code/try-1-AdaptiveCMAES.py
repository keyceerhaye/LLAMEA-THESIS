...
class AdaptiveCMAES:
    ...
    def __call__(self, func):
        ...
        for _ in range(self.budget // self.population_size):
            ...
            # New lines for adaptive mutation and mirrored sampling
            if np.random.rand() < 0.5:
                y_mirror = np.dot(-z, np.linalg.cholesky(self.covariance_matrix).T)
                x_mirror = self.mean + self.sigma * y_mirror
                x_mirror = np.clip(x_mirror, lb, ub)
                f_values_mirror = np.array([func(ind) for ind in x_mirror])
                self.sigma *= 0.9 if f_values_mirror.min() < f_values.min() else 1.1
