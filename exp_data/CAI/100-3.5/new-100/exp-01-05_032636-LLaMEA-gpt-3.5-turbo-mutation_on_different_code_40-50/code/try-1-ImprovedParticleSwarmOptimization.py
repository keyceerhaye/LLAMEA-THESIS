...
class ImprovedParticleSwarmOptimization:
    def __init__(self, budget=10000, dim=10, num_particles=30, inertia_weight=0.5, cognitive_weight=1.5, social_weight=1.0, inertia_decay=0.99, c1=2.0, c2=2.0):
        self.budget = budget
        self.dim = dim
        ...
        for _ in range(self.budget):
            for i in range(self.num_particles):
                vel = self.inertia_weight * particles_vel[i] + \
                      self.cognitive_weight * self.c1 * np.random.rand() * (personal_best_pos[i] - particles_pos[i]) + \
                      self.social_weight * self.c2 * np.random.rand() * (global_best_pos - particles_pos[i])
