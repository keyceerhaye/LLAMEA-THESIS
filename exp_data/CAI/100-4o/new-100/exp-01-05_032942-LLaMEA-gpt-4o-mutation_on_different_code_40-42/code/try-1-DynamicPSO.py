...
class DynamicPSO:
    ...
    def __call__(self, func):
        ...
        neighborhood_best_values = personal_best_values.copy()
        neighborhood_bests = personal_bests.copy()

        for i in range(self.budget):
            w = self.w_max - (self.w_max - self.w_min) * (i / self.budget) ** 2  # Adaptive inertia
            ...
            for j in range(self.swarm_size):
                ...
                neighborhood_best_values[j] = min(personal_best_values[neighbors])
                neighborhood_bests[j] = personal_bests[neighbors[np.argmin(personal_best_values[neighbors])]]

            for j in range(self.swarm_size):
                inertia = w * velocities[j]
                cognitive = self.c1 * np.random.rand(self.dim) * (personal_bests[j] - particles[j])
                social = self.c2 * np.random.rand(self.dim) * (neighborhood_bests[j] - particles[j])
