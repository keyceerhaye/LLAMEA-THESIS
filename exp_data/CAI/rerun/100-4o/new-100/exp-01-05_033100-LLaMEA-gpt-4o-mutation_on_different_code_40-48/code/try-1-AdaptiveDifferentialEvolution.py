...
class AdaptiveDifferentialEvolution:
    ...
    def _mutate(self, population, target_idx):
        ...
        chaotic_factor = np.random.logistic(0, 1)  # Chaotic sequence
        mutant = a + self.F * (b - c) * chaotic_factor
    ...
    def _adaptive_control(self, success_rate):
        if success_rate < 0.2:
            self.F *= 0.8
            self.CR *= 0.9
            self.pop_size = max(self.pop_size - 1, 4 * self.dim)  # Reduce population
