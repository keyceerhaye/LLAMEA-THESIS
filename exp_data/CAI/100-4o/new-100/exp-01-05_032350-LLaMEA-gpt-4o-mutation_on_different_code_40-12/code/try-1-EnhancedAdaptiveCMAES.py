...
class EnhancedAdaptiveCMAES:
    def __init__(self, budget=10000, dim=10):
        ...
        self.cs = (self.mueff + 4) / (dim + self.mueff + 5)  # Adjusted cs
    ...
    def __call__(self, func):
        xmean = np.random.uniform(-2.0, 2.0, self.dim)  # Narrower initialization range
        sigma = 0.2  # Reduced initial step size
        counteval = 0

        while counteval < self.budget:
            self.lambda_ = max(4 + int(3 * np.log(self.dim)), int(self.lambda_ * 0.98))  # Adaptive population size
            ...
            sigma *= np.exp((self.cs / self.damps) * (np.linalg.norm(self.ps) / self.chiN - 1.2))  # Slightly altered scaling
