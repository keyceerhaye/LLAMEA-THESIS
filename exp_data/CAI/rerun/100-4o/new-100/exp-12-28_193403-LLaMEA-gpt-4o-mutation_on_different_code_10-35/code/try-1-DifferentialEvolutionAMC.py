...
class DifferentialEvolutionAMC:
    def __init__(self, budget=10000, dim=10, population_size=50):
        ...
        self.memory_factor = 0.9  # Memory factor for successful mutation factors
    ...
    def __call__(self, func):
        ...
        evals = self.pop_size
        successful_mutations = np.full(self.pop_size, 0.5)  # Memory for successful strategies
        ...
        while evals < self.budget:
            for i in range(self.pop_size):
                ...
                if f_trial < fitness[i]:
                    population[i] = trial
                    fitness[i] = f_trial
                    successful_mutations[i] = self.mutation_factor[i]  # Store successful mutation factor
                    self.mutation_factor[i] = self.memory_factor * successful_mutations[i] + 0.5 * np.random.uniform(0.5, 1.0)
