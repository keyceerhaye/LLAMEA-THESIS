...
class Solution:
    ...
    # MADA helpers
    # ------------------------------------------------------------------ #
    def set_mada_blocks(self, blocks):
        """Store parsed MADA blocks inside metadata."""

        self.metadata["mada_blocks"] = blocks or {}

    def get_mada_blocks(self):
        """Return stored MADA blocks if available."""

        return self.metadata.get("mada_blocks")

    def set_lineage(self, lineage):
        """Persist the latest MADA lineage trace."""

        self.metadata["mada_lineage"] = lineage or {}

    def get_lineage(self):
        """Return the recorded MADA lineage if one exists."""

        return self.metadata.get("mada_lineage")

    def mark_mada_reward_ready(self, ready: bool):
        """Mark whether the DS-TS reward has been propagated."""

        self.metadata["mada_reward_ready"] = bool(ready)

    def mada_reward_ready(self) -> bool:
        """Check if reward propagation already happened."""

        return bool(self.metadata.get("mada_reward_ready"))

    def clear_mada_metadata(self):
        """Drop any stored MADA metadata."""

        for key in ("mada_blocks", "mada_lineage", "mada_reward_ready"):
