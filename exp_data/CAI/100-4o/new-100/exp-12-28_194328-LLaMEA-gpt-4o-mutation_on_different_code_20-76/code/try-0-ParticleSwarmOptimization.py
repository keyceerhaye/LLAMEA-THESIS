...
class ParticleSwarmOptimization:
    ...
    def __call__(self, func):
        ...
        velocity_clamp = (lb - ub) * 0.1
        
        # Initialize swarm
        swarm = np.random.uniform(lb, ub, (self.swarm_size, self.dim))
        velocities = np.random.uniform(-abs(velocity_clamp), abs(velocity_clamp), (self.swarm_size, self.dim))
        ...
        for _ in range(self.budget // self.swarm_size):
            for i in range(self.swarm_size):
                # Update velocity
                inertia = 0.5 + np.random.rand() / 2  # Dynamic inertia
                cognitive_component = np.random.rand(self.dim)
