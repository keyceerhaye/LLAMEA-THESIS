    ...
    def __call__(self, func):
        ...
        while num_evaluations < self.budget:
            for i in range(self.pop_size):
                ...
                if num_evaluations % (self.budget // 10) == 0 and self.pop_size < 50:
                    self.pop_size += 1
                    population = np.vstack([population, np.random.uniform(func.bounds.lb, func.bounds.ub, (1, self.dim))])
                    fitness = np.append(fitness, func(population[-1]))

            # Adapt parameters
            self.F = np.clip(self.F + np.random.normal(0, 0.1), 0.1, 1.0)
            self.CR = np.clip(self.CR + 0.1 * (fitness < np.median(fitness)).mean(), 0.1, 1.0)
