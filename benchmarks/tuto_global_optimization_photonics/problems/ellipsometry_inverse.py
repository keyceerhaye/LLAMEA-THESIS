...
class ellipsometry(photonic_problem):
    def __init__(self, mat_env, mat_substrate, nb_layers, min_thick, max_thick,
        ...
        self.angle = angle
        self.lb = np.array([self.min_eps]*self.nb_layers +
                           [self.min_thick]*self.nb_layers)
        self.ub = np.array([self.max_eps]*self.nb_layers +
                           [self.max_thick]*self.nb_layers)
    ...
    def setup_structure(self, x):
        ...
        materials = [self.mat_env] + \
            [_m for _m in x[:self.nb_layers]] + [self.mat_substrate]
