import numpy as np

class FireflyAlgorithm:
    def __init__(self, budget=10000, dim=10, alpha=0.5, beta0=2.0, gamma=1.0):
        self.budget = budget
        self.dim = dim
        self.alpha = alpha
        self.beta0 = beta0
        self.gamma = gamma
        self.f_opt = np.Inf
        self.x_opt = None

    def attractiveness(self, x, y):
        return np.exp(-self.gamma * np.linalg.norm(x - y)**2)

    def move_firefly(self, x, y, beta):
        epsilon = np.random.uniform(-1, 1, size=self.dim)
        return x + self.alpha * np.exp(-self.beta0 * np.linalg.norm(x - y)) * (y - x) + beta * epsilon

    def __call__(self, func):
        fireflies = np.random.uniform(func.bounds.lb, func.bounds.ub, size=(self.budget, self.dim))

        for i in range(self.budget):
            f_cur = func(fireflies[i])
            for j in range(self.budget):
                if func(fireflies[j]) < f_cur:
                    beta = np.random.uniform(0, 1)
                    fireflies[i] = self.move_firefly(fireflies[i], fireflies[j], beta)

        index_min = np.argmin([func(f) for f in fireflies])
        self.f_opt = func(fireflies[index_min])
        self.x_opt = fireflies[index_min]

        return self.f_opt, self.x_opt