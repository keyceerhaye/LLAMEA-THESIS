    ...
    def __call__(self, func):
        ...
        while n_evals < self.budget:
            ...
            for i in range(self.pop_size):
                indices = np.random.choice(self.pop_size, 4, replace=False)  # Changed from 3 to 4 for increased diversity
                x1, x2, x3, x4 = population[indices]
                
                mutant = np.clip(x1 + F * (x2 - x3 + x4 - x1), lb, ub)  # Adjusted mutation strategy
