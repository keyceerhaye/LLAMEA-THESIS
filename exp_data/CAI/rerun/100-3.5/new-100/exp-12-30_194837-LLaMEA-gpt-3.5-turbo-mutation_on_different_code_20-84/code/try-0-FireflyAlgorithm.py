...
class FireflyAlgorithm:
    ...
    def move(self, x, x_neighbor):
        beta = self.beta0 * np.exp(-self.gamma * np.linalg.norm(x - x_neighbor))
        return x + self.alpha * (x_neighbor - x) + beta * np.random.uniform(-1, 1, self.dim)
