...
class HarmonySearch:
    def __init__(self, budget=10000, dim=10, harmony_memory_size=20, base_pitch_adjust_rate=0.3):
        self.budget = budget
        self.dim = dim
        self.harmony_memory_size = harmony_memory_size
        self.base_pitch_adjust_rate = base_pitch_adjust_rate
        self.f_opt = np.Inf
        self.x_opt = None
    ...
    def generate_new_harmony(self):
        ...
        for i in range(self.dim):
            pitch_adjust_rate = self.base_pitch_adjust_rate * np.abs(new_harmony[i] - self.x_opt[i])
