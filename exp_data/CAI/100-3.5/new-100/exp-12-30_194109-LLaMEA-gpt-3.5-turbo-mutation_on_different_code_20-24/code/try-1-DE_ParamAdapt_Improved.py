...
class DE_ParamAdapt_Improved:
    ...
    def __call__(self, func):
        ...
        for i in range(self.budget):
            F_curr = self.F * np.exp(-i / self.budget)  # Dynamic F
            CR_curr = self.CR * np.exp(-i / self.budget)  # Dynamic CR
            
            for j in range(len(population)):
                indices = np.random.choice(len(population), 3, replace=False)
                x_r1, x_r2, x_r3 = population[indices]
                
                mutant_vector = population[j] + F_curr * (x_r1 - population[j]) + F_curr * (x_r2 - x_r3)
