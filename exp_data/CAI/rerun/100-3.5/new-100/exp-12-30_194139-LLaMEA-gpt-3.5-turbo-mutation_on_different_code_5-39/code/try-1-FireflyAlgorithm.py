class FireflyAlgorithm:
    ...
    def move_fireflies(self, fireflies, func, iter_count):
        for i in range(len(fireflies)):
            for j in range(len(fireflies)):
                if func(fireflies[j]) < func(fireflies[i]):
                    r = np.linalg.norm(fireflies[j] - fireflies[i])
                    beta = self.attractiveness(r)
                    fireflies[i] = fireflies[i] + beta * (fireflies[j] - fireflies[i]) + self.alpha * np.random.uniform(-1, 1, self.dim) / np.sqrt(iter_count)
        return fireflies
    ...
    def __call__(self, func):
        ...
        for iter_count in range(1, self.budget + 1):
            fireflies = self.move_fireflies(fireflies, func, iter_count)
