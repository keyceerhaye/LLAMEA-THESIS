...
class AdaptivePSO:
    def __init__(self, budget=10000, dim=10, num_particles=30):
        ...
        self.f_opt = np.Inf
        self.x_opt = None
        self.v_max = 0.1 * (func.bounds.ub - func.bounds.lb)  # Velocity clamping factor
        self.k = 0.729  # Constriction factor

    def __call__(self, func):
        bounds = (func.bounds.lb, func.bounds.ub)
        particles = np.random.uniform(bounds[0], bounds[1], (self.num_particles, self.dim))
        velocities = np.random.uniform(-self.v_max, self.v_max, (self.num_particles, self.dim))  # Adjust initial velocities
        ...
        while evaluations < self.budget:
            ...
            velocities = self.k * (w * velocities +
