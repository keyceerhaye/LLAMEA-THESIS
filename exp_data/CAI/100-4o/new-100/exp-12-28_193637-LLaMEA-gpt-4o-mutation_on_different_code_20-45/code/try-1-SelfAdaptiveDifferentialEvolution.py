    ...
    def __call__(self, func):
        ...
        while func_evals < self.budget:
            for i in range(self.pop_size):
                ...
                diversity = np.std(self.population, axis=0)  # Calculate diversity
                ...
                if score_trial < scores[i]:
                    ...
                    self.mutation_scale[i] = np.clip(self.mutation_scale[i] * (1 + 0.1 * (np.mean(diversity) > 0.1)), 0.5, 1.0)
                    self.crossover_rate[i] = np.clip(self.crossover_rate[i] * (1 - 0.1 * (score_trial < self.f_opt * 1.01)), 0.1, 0.9)
