...
class grating2D(photonic_problem):
    def __init__(self, nb_layers, min_w, max_w, min_thick, max_thick,
                 min_p, max_p):
        super().__init__()
        ...
        self.min_p = min_p
        self.max_p = max_p
        self.lb = np.array([min_w] * nb_layers + [min_thick] * nb_layers +
                           [min_p] * nb_layers)
        self.ub = np.array([max_w] * nb_layers + [max_thick] * nb_layers +
                           [max_p] * nb_layers)
    # i = complex(0, 1)
    ...
    def cascade(self, T, U):
        ...
        S = np.block([[T[0:n, 0:n]+np.matmul(np.matmul(np.matmul(T[0:n, n:2*n], J), U[0:n, 0:n]), T[n:2*n, 0:n]), np.matmul(np.matmul(T[0:n, n:2*n], J), U[0:n, n:2*n])],
                     [np.matmul(np.matmul(U[n:2*n, 0:n], K), T[n:2*n, 0:n]), U[n:2*n, n:2*n]+np.matmul(np.matmul(np.matmul(U[n:2*n, 0:n], K), T[n:2*n, n:2*n]), U[0:n, n:2*n])]])
        return S

    def c_bas(self, A, V, h):
        n = int(A.shape[1]/2)
        D = np.diag(np.exp(1j*V*h))
        S = np.block([[A[0:n, 0:n], np.matmul(A[0:n, n:2*n], D)], [np.matmul(D,
                     A[n:2*n, 0:n]), np.matmul(np.matmul(D, A[n:2*n, n:2*n]), D)]])
        return S

    def marche(self, a, b, p, n, x):
        l = np.zeros(n, dtype=np.complex128)
        m = np.zeros(n, dtype=np.complex128)
        tmp = 1/(2*np.pi*np.arange(1, n))*(np.exp(-2*1j*np.pi*p *
                                                  np.arange(1, n))-1)*np.exp(-2*1j*np.pi*np.arange(1, n)*x)
        l[1:n] = 1j*(a-b)*tmp
        l[0] = p*a+(1-p)*b
        m[0] = l[0]
    ...
    def creneau(self, k0, a0, pol, e1, e2, a, n, x0):
        ...
        if (pol == 0):
            ...
            T = np.linalg.inv(U)
            M = np.matmul(np.matmul(np.matmul(T, alpha), np.linalg.inv(
                self.marche(e1, e2, a, n, x0))), alpha)-k0*k0*T
            L, E = np.linalg.eig(M)
            L = np.sqrt(-L+0j)
            L = (1-2*(np.imag(L) < -1e-15))*L
    ...
    def homogene(self, k0, a0, pol, epsilon, n):
        nmod = int(n/2)
        valp = np.sqrt(epsilon*k0*k0-(a0+2*np.pi *
                       np.arange(-nmod, nmod+1))**2+0j)
    ...
    def interface(self, P, Q):
        n = int(P.shape[1])
        S = np.matmul(np.linalg.inv(np.block([[P[0:n, 0:n], -Q[0:n, 0:n]], [P[n:2*n, 0:n], Q[n:2*n, 0:n]]])), np.block(
    ...
    def __call__(self, x):
        ...
        S = np.block([[np.zeros([n, n]), np.eye(n, dtype=np.complex128)], [
