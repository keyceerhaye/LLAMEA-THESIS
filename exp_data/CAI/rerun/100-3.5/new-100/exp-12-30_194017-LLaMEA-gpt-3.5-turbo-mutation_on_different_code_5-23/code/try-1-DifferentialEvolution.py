class DifferentialEvolution:
    def __init__(self, budget=10000, dim=10, F=0.5, CR=0.9):
        self.budget = budget
        self.dim = dim
        self.F = F  # Differential weight
        self.CR = CR  # Crossover probability
        self.f_opt = np.Inf
        self.x_opt = None

    def __call__(self, func):
        population = np.random.uniform(-5.0, 5.0, size=(self.budget, self.dim))
        fitness = np.array([func(x) for x in population])
        
        decay_factor = 0.95  # Adaptive decay factor for mutation parameter
        F_min = 0.1  # Minimum mutation parameter
        
        for i in range(self.budget):
            for j in range(len(population)):
                indices = [idx for idx in range(len(population)) if idx != j]
                a, b, c = population[np.random.choice(indices, 3, replace=False)]
                
                F_current = max(F_min, self.F * decay_factor ** (i / self.budget))
                mutant = np.clip(a + F_current * (b - c), -5.0, 5.0)

                crossover = np.random.rand(self.dim) < self.CR
                trial = np.where(crossover, mutant, population[j])
                
                f_trial = func(trial)
                if f_trial < fitness[j]:
                    population[j] = trial
                    fitness[j] = f_trial
                
                if fitness[j] < self.f_opt:
                    self.f_opt = fitness[j]
                    self.x_opt = population[j]

        return self.f_opt, self.x_opt