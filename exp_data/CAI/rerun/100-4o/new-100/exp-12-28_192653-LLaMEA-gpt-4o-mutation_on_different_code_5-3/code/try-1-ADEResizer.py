...
class ADEResizer:
    def __init__(self, budget=10000, dim=10):
        ...
        self.stall_count = 0 # Counter for restart mechanism
        self.restart_threshold = 100 # Threshold to trigger restart
        ...
        while evaluations < self.budget:
            for i in range(self.num_individuals):
                ...
                if trial_fitness < fitness[i]:
                    ...
                    success = True
                    self.stall_count = 0  # Reset stall counter on success
                else:
                    success = False
                    self.stall_count += 1  # Increment stall counter on failure
                
                self.adapt_parameters(success, not success)
                self.resize_population(success)
                
                if self.stall_count > self.restart_threshold:  # Check if restart is needed
                    population = np.random.uniform(self.lower_bound, self.upper_bound, (self.num_individuals, self.dim))
                    fitness = np.array([func(ind) for ind in population])
                    self.stall_count = 0
