...
class EnhancedGreyWolfOptimization:
    ...
    def adapt_population_size(self, population, func):
        fitness = np.array([func(x) for x in population])
        ...
        if np.min(fitness) < threshold:
            ...
            self.population_size = min(self.population_size, 20)  # Limit maximum population size
    ...
    def __call__(self, func):
        ...
        for _ in range(self.budget):
            ...
            self.population_size = self.adapt_population_size(population, func)
            fitness = np.array([func(x) for x in population])
