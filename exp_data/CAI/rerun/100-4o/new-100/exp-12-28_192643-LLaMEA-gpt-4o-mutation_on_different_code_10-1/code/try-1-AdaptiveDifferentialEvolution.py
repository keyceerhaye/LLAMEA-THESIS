...
class AdaptiveDifferentialEvolution:
    ...
    def adapt_params(self):
        ...
        self.pop_size = max(5, int(20 * (1 - self.func_evals / self.budget)))  # Dynamic pop size

    def local_search(self, x):
        perturbation = np.random.normal(0, 0.1, self.dim)
        neighbor = np.clip(x + perturbation, -5.0, 5.0)
        return neighbor
        ...
        while self.func_evals < self.budget:
            for i in range(self.pop_size):
                ...
                if np.random.rand() < 0.1:
                    neighbor = self.local_search(population[i])
