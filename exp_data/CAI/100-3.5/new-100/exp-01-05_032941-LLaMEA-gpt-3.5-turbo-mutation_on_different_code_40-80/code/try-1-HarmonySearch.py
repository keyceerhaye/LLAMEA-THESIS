...
class HarmonySearch:
    def __init__(self, budget=10000, dim=10, harmony_memory_size=10, hmcr=0.7, par=0.4, bandwidth=0.01, bandwidth_min=0.001, bandwidth_max=0.1, bandwidth_adjust_rate=0.9):
        self.budget = budget
        self.dim = dim
        ...
        self.bandwidth_adjust_rate = bandwidth_adjust_rate
        self.f_opt = np.Inf
        self.x_opt = None
    ...
    def __call__(self, func):
        ...
        for i in range(self.budget):
            if np.random.rand() < self.hmcr:
                ...
                self.bandwidth = np.clip(self.bandwidth, self.bandwidth_min, self.bandwidth_max)
            else:
