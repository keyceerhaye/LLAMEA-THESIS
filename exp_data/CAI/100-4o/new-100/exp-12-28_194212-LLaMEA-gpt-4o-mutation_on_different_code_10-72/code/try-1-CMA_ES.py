...
class CMA_ES:
    def __init__(self, budget=10000, dim=10):
        ...
        self.f_opt = np.Inf
        self.x_opt = None
        self.sigma = 0.5  # Adjusted step size
    ...
    def __call__(self, func):
        ...
        for _ in range(self.budget // self.population_size):
            ...
            # Use a more diverse set of samples for covariance adaptation
            self.C = (1 - self.c1 - self.c_mu) * self.C + self.c1 * (np.outer(self.pc, self.pc) + (1 - hsig) * self.cc * (2 - self.cc) * self.C) \
                     + self.c_mu * np.sum([w * np.outer(p, p) for w, p in zip(self.weights[:self.population_size], population[:self.population_size])], axis=0)

            # Update step-size with a more dynamic strategy
            self.sigma *= np.exp((self.cs / self.damps) * (np.linalg.norm(self.ps) / norm_z - 0.8))
