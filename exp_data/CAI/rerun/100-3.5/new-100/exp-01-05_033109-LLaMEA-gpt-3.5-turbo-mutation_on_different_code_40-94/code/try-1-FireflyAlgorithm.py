...
class FireflyAlgorithm:
    def __init__(self, budget=10000, dim=10, alpha=0.5, beta0=1.0, gamma=0.01, elitism_rate=0.1):
        self.budget = budget
        self.dim = dim
        self.alpha = alpha
        self.beta0 = beta0
        ...
        self.best_solution = np.copy(self.population[self.best_fitness_idx])
    ...
    def __call__(self, func):
        for t in range(self.budget):
            ...
            elite_count = int(self.elitism_rate * self.budget)
            elite_indices = np.argsort(self.fitness)[:elite_count]
            for elite_idx in elite_indices:
                if self.fitness[elite_idx] < func(self.best_solution):
                    self.best_solution = np.copy(self.population[elite_idx])

        return func(self.best_solution), self.best_solution