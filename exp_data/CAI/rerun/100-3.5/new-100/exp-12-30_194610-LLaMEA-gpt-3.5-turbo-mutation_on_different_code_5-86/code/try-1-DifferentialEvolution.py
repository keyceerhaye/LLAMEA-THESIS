class DifferentialEvolution:
    def __init__(self, budget=10000, dim=10, F=0.5, CR=0.9, pop_size=30, F_decay=0.9, F_decay_step=1000):
        self.budget = budget
        self.dim = dim
        self.F = F
        self.CR = CR
        ...
        self.F_decay_step = F_decay_step
        self.f_opt = np.Inf
        self.x_opt = None
    ...
    def __call__(self, func):
        ...
        for i in range(self.budget):
            if i % self.F_decay_step == 0 and i > 0:
                self.F *= self.F_decay
