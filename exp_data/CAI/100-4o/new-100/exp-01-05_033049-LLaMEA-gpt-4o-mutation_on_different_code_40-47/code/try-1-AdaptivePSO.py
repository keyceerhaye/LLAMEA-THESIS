...
class AdaptivePSO:
    ...
    def __call__(self, func):
        ...
        for iteration in range(self.budget // self.swarm_size):
            ...
            if global_best_position is None or self.f_opt < func(global_best_position):
            ...
            diversity_factor = np.std(personal_best_scores) / np.mean(personal_best_scores)
            velocities = inertia_weight * velocities + \
                         cognitive_coeff * np.random.rand(self.swarm_size, self.dim) * (personal_best_positions - particles) + \
                         social_coeff * np.random.rand(self.swarm_size, self.dim) * (global_best_position - particles) * diversity_factor
