    ...
    def __call__(self, func):
        ...
        while evaluations < self.budget:
            for i in range(self.population_size):
                ...
                adaptive_mutation_factor = self.mutation_factor * (1 - progress_ratio) + 0.3 * progress_ratio
                mutant = self.pop[a] + adaptive_mutation_factor * (self.pop[b] - self.pop[c])
                mutant = np.clip(mutant, -5.0, 5.0)

                # Dynamic crossover rate
                cross_points = np.random.rand(self.dim) < (self.crossover_rate + progress_ratio * 0.2)
