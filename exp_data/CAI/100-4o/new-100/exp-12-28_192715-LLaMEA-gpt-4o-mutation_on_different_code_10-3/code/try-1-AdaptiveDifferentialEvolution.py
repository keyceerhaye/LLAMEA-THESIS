...
class AdaptiveDifferentialEvolution:
    ...
    def __call__(self, func):
        ...
        for _ in range(self.budget - self.population_size):
            ...
            population_size_adjustment = int(0.05 * self.population_size)
            if np.random.rand() < 0.5:  # Add individuals
                new_individuals = np.random.uniform(self.bounds[0], self.bounds[1],
                                                    (population_size_adjustment, self.dim))
                population = np.vstack((population, new_individuals))
                new_fitness = np.array([func(ind) for ind in new_individuals])
                fitness = np.concatenate((fitness, new_fitness))
            else:  # Retain best individuals
