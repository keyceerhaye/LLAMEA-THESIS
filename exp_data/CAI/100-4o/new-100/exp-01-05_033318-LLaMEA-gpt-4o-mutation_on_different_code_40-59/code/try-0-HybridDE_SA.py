    ...
    def __call__(self, func):
        ...
        while evals < self.budget:
            for i in range(self.pop_size):
                ...
                mutant = np.clip(pop[a] + self.F * (pop[b] - pop[c]), func.bounds.lb, func.bounds.ub)
            ...
            for i in range(self.pop_size):
                ...
                # Accept with probability if perturbed solution is better or based on temperature
                if perturbed_fitness < fitness[i] or np.exp(-(perturbed_fitness - fitness[i]) / self.T) > np.random.rand():
                    pop[i] = perturbed
