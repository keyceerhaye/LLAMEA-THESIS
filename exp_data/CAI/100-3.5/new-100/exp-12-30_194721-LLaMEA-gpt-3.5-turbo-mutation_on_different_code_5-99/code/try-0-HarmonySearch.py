    ...
    def __call__(self, func):
        ...
        for i in range(self.budget):
            if np.random.rand() < self.hmcr:
                idx = np.random.choice(range(i))
                for j in range(self.dim):
                    if np.random.rand() < self.par:
                        harmony_memory[i, j] = harmony_memory[idx, j]
            else:
                harmony_memory[i] = np.random.uniform(func.bounds.lb, func.bounds.ub, size=self.dim)

            harmony_memory[i] += np.random.uniform(-self.bw, self.bw, size=self.dim)
            harmony_memory[i] = np.clip(harmony_memory[i], func.bounds.lb, func.bounds.ub)
