...
class HybridPSOSA:
    ...
    def __call__(self, func):
        ...
        while eval_count < self.budget:
            ...
            # Adjust cognitive and social parameters based on progress
            if eval_count % 100 == 0:  # every 100 evaluations
                best_score_improvement = np.min(personal_best_scores) - self.f_opt
                if best_score_improvement < 0.01:
                    self.cognitive *= 0.95
                    self.social *= 1.05
                else:
                    self.cognitive *= 1.05
