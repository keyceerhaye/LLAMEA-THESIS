...
def find_best_alg(list_of_dirs):
    ...
    for dir_name in list_of_dirs:
        log_file = f"exp_data/CAI/clip/{dir_name}/log.jsonl"
        aucs = []
        with open(log_file, "r") as reader:
            for line in reader:
                obj = json.loads(line)
                if obj["metadata"] != {}:
                    aucs += [np.mean(obj["metadata"]["final_y"])]
                else:
                    aucs += [np.inf]
        max_auc = min(aucs)
    ...
    best_alg_auc = min(max_aucs)
    best_alg_idx = max_aucs.index(best_alg_auc)
    return list_of_dirs[best_alg_idx], max_aucs_idx[best_alg_idx]


exp_names = ["Photonic01", "Photonic02", "Photonic03"]  # , "Photonic03-extend"
dirs = os.listdir("exp_data/CAI/clip")
