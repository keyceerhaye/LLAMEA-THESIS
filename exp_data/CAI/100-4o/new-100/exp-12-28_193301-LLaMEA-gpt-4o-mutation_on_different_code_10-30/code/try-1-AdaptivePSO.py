...
class AdaptivePSO:
    def __init__(self, budget=10000, dim=10, num_particles=30):
        ...
        self.w_max = 0.9
        self.w_min = 0.4
        self.c1_max = 2.5  # Increased max value for c1
        self.c1_min = 0.5  # Decreased min value for c1
        self.c2_max = 0.5  # Decreased max value for c2
        self.c2_min = 2.5  # Increased min value for c2
    ...
    def __call__(self, func):
        ...
        for i in range(self.budget // self.num_particles):
            ...
            c1 = self.c1_max - (self.c1_max - self.c1_min) * i / (self.budget // self.num_particles)
            c2 = self.c2_min + (self.c2_max - self.c2_min) * i / (self.budget // self.num_particles)
            for j in range(self.num_particles):
                r1, r2 = np.random.rand(self.dim), np.random.rand(self.dim)
                velocities[j] = (w * velocities[j] +
                                 c1 * r1 * (p_best_positions[j] - particles[j]) +
                                 c2 * r2 * (g_best_position - particles[j]))
