...
class CooperativeSwarmOptimization:
    ...
    def __call__(self, func):
        for iter_count in range(self.budget // self.swarm_size):
            # Adaptive inertia weight and mutation factor
            inertia_weight = 0.9 - iter_count * ((0.9 - 0.4) / (self.budget // self.swarm_size))
            mutation_factor = 0.8 + iter_count * ((1.2 - 0.8) / (self.budget // self.swarm_size))
            ...
            # Differential evolution style exploration with adaptive mutation factor
            for i in range(self.swarm_size):
                idxs = [idx for idx in range(self.swarm_size) if idx != i]
                a, b, c = np.random.choice(idxs, 3, replace=False)
                mutant = np.clip(self.pos[a] + mutation_factor * (self.pos[b] - self.pos[c]), -5.0, 5.0)
