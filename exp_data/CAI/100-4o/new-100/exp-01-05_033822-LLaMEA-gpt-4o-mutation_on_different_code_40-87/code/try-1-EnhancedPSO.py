...
class EnhancedPSO:
    ...
    def __call__(self, func):
        ...
        evaluations = 0
        adapt_c1, adapt_c2 = self.c1, self.c2
        while evaluations < self.budget:
            ...
            adapt_c2 = self.c2 * (evaluations / self.budget)
            self.inertia_weight = (self.inertia_max - self.inertia_min) * (self.budget - evaluations) / self.budget + self.inertia_min
            velocities = (self.inertia_weight * velocities
                          + adapt_c1 * r1 * (personal_best_positions - positions)
                          + adapt_c2 * r2 * (global_best_position - positions))
