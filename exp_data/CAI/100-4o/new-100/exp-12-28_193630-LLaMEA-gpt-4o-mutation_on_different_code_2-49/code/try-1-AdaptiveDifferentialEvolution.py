    ...
    def __call__(self, func):
        ...
        while eval_count < self.budget:
            ...
            if eval_count % (self.pop_size * 5) == 0:  # Adjust parameters adaptively
                ...
                # Reset a random subset of the population for diversification
                reset_indices = np.random.choice(self.pop_size, size=int(self.pop_size * 0.1), replace=False)
                population[reset_indices] = np.random.uniform(lb, ub, (len(reset_indices), self.dim))
                fitness[reset_indices] = [func(ind) for ind in population[reset_indices]]
