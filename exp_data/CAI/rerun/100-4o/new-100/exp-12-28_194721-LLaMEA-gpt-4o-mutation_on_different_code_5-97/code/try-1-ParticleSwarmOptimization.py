...
class ParticleSwarmOptimization:
    ...
    def __call__(self, func):
        ...
        for eval_count in range(0, self.budget, self.swarm_size):
            ...
            inertia = 0.5 + (0.9 - 0.5) * np.power(1 - eval_count / self.budget, 2)
            for i in range(self.swarm_size):
                cognitive_component = np.random.rand(self.dim) * (personal_best_positions[i] - particles[i])
                social_component = 2.5 * np.random.rand(self.dim) * (global_best_position - particles[i])
                velocities[i] = inertia * velocities[i] + 2.0 * cognitive_component + social_component
