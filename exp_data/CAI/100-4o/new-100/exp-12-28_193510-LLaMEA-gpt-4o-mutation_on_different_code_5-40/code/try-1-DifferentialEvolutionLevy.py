...
class DifferentialEvolutionLevy:
    ...
    def mutate(self, idx):
        ...
        # Adjusted mutation factor F based on fitness variance
        dynamic_F = self.F * (1 + np.std(self.fitness) / np.mean(self.fitness))
        mutant_vector = np.clip(self.population[a] + dynamic_F * (self.population[b] - self.population[c]), 
